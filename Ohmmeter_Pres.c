...
void Ohmmeter_Pres_poll(void)
{
    ...
    switch (state)
    {
        ...
        case 1:
            ...
            vout1 = ((float)adcVal1 * OHM_Pres_VIN / OHM_Pres_ADC_RES) + OHM_Pres_ADJ;
            vout2 = ((float)adcVal2 * OHM_Pres_VIN / OHM_Pres_ADC_RES) + OHM_Pres_ADJ;

            float vTotal = vout1 + vout2;
            if (vout1 > 0.001f && vout2 > 0.001f)
            {
                rx1 = ((OHM_Pres_R1 * vTotal) / vout1) - OHM_Pres_R1;
                rx2 = OHM_Pres_R1 / ((vTotal / vout2) - 1.0f);
